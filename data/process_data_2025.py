...
def survey_field_exceptions(clean_lab_id):
    

    # MO, just goofed on the dairyland sample label
    if clean_lab_id in ["00002-25-S", "00002-25-F"]:
        survey_field = SurveyField.objects.get(id = 282)
        return survey_field
        
    
    # JD
    # These should maybe be attached to same field
    ...
    if clean_lab_id == "00032-25-F":
        survey_field = SurveyField.objects.get(id = 246)
        return survey_field

    # JK
    if clean_lab_id == "00085-25-F":
        # Crimson mix
        survey_field = SurveyField.objects.get(id = 286)
        return survey_field
    
    if clean_lab_id == "00055-25-F":
        # Cowpea mix
        survey_field = SurveyField.objects.get(id = 284)
        return survey_field

    return None

def find_survey_field(clean_lab_id, farmer_id, survey_year = 2025):

    survey_field = survey_field_exceptions(clean_lab_id)
...
def process_dl_2025(fl_dairyland_2025):
    ...
    for i, row in dat.iterrows():
        ...
        farmer_name = f"{farmer.first_name} {farmer.last_name}"
        print(farmer_name)
        survey_field = find_survey_field(clean_lab_id, farmer_id, survey_year = 2025)
        if survey_field:
