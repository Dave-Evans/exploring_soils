...
################################################################################
## For 2022 Data ###############################################################
dir_data <- "~/Documents/small_projects/wisc_cc/data_from_mrill/"
dir_lab_extract <- "./data/labdata_2022/"

fl_agronomic_2022 <- "CitSci CCROP 2022 responses agronomic.xlsx"
fl_gdd_2022 <- "CitSci_GDU_pcp_2022_gs.xlsx"
fl_biomass_forage_2022 <- "Table 1. DS draft 2.13.22 CCROP Citizen Science Data 2022 with termination.xlsx" # nolint: line_length_linter.
fl_species_2022 <- "Table 2. for CCROP 2022 (w ID).xlsx"
dat_survey <- read_excel(
  paste0(dir_data, fl_agronomic_2022),
  sheet = 1
)
dat_gdd <- read_excel(
  paste0(dir_data, fl_gdd_2022),
  sheet = 1
)
dat_biomass <- read_excel(
  paste0(dir_data, fl_biomass_forage_2022),
  sheet = 1,
  skip = 1
)
dat_forage_q <- read_excel(
  paste0(dir_data, fl_biomass_forage_2022),
  sheet = 2,
  range = "A1:I59"
)
dat_forage_q_addl_agsource <- read_csv(
  paste0(dir_lab_extract, "agsource_output.csv")
)
dat_forage_q_addl_dairyland <- read_csv(
  paste0(dir_lab_extract, "dairyland_output.csv")
)
dat_species <- read_excel(
  paste0(dir_data, fl_species_2022),
  sheet = 1
)

# Looking for duplicates
# nrow(dat_survey) == length(unique(dat_survey$`ID (zipcode-initials-signup year)`))
# nrow(dat_gdd) == length(unique(dat_gdd$id))
# nrow(dat_biomass) == length(unique(dat_biomass$`...1`))
# nrow(dat_forage_q) == length(unique(dat_forage_q$`...1`))
# nrow(dat_species) == length(unique(dat_species$`ID#`))
# nrow(dat_forage_q_addl_agsource) == length(unique(dat_forage_q_addl_agsource$cleaned_id))
# nrow(dat_forage_q_addl_dairyland) == length(unique(dat_forage_q_addl_dairyland$cleaned_id))

dat_2022 <- dat_survey %>%
  left_join(
    dat_gdd,
    by = join_by("ID (zipcode-initials-signup year)" == "id")
  ) %>%
  left_join(
    dat_biomass,
    by = join_by("ID (zipcode-initials-signup year)" == `...1`)
  ) %>%
  left_join(
    dat_forage_q,
    by = join_by("ID (zipcode-initials-signup year)" == `...1`)
  ...
  left_join(
    dat_forage_q_addl_agsource,
    by = join_by("ID (zipcode-initials-signup year)" == "cleaned_id")
  ) %>%
  left_join(
    dat_forage_q_addl_dairyland,
    by = join_by("ID (zipcode-initials-signup year)" == "cleaned_id")
  ) %>%
  rename(id = "ID (zipcode-initials-signup year)") %>%
  rename(
    old_id = "old version of our ID system (zipcode-initials-signup year)"
  ) %>%
  rename(
    county = "1. In what county do you farm? (If you farm in more than one, list them in order of number of acres.)"
  ) %>%
  rename(
    years_experience = "3. How many total years experience do you have planting cover crops?"
  ) %>%
  rename(
    zipcode = "15. Closest zip code for this field (so we can determine appropriate climate data and generate a location map of participating fields). Field must be located in Wisconsin."
  ) %>%
  rename(
    cc_species_raw = "18. Please select any of the following that were planted as a cover crop in this field this year."
  ) %>%
  rename(
    previous_crop = "19. Previous crop in field"
  ) %>%
  rename(
    cash_crop_planting_date = "20. What date this year did you plant your cash crop in this field?"
  ) %>%
  rename(
    dominant_soil_texture = "22. Please choose the dominant soil texture of the field."
  ) %>%
  rename(
    manure_prior = "23. Will you apply manure prior to seeding cover crops on this field?"
  ) %>%
  rename(
    manure_post = "24. Will manure be applied to the field after the cover crop is established?"
  ) %>%
  rename(
    manure_rate = "25. What manure application rate are you targeting to apply prior to or after establishing your cover crop?"
  ) %>%
  rename(
    manure_value = "25a. Type in number corresponding to selection above in 25."
  ) %>%
  rename(
    tillage_system = "26. What is your tillage system for the cash crop preceding the cover crop?"
  ) %>%
  rename(
    tillage_equip_primary = "26a.  Primary tillage equipment (select all that apply) for a cash crop preceding a cover crop?"
  ) %>%
  rename(
    tillage_equip_secondary = "26b. Secondary tillage equipment (select all that apply) for cash crop preceding the cover crop?"
  ) %>%
  rename(
    soil_conditions = "27. Soil conditions in this field at cover crop seeding"
  ) %>%
  rename(
    cc_seeding_method = "28. Cover Crop Seeding Method."
  ) %>%
  rename(
    cc_planting_rate = "29. At what rate did you plant your cover crops (please type species and pounds per acre)."
  ) %>%
  rename(
    cc_termination = "33. Estimated termination timing for this field."
  ) %>%
  rename(
    days_between_crop_hvst_and_cc_estd = "34. Number of days estimated between crop harvest and cover crop establishment in this field."
  ) %>%
  # Growing degree days
  rename(cc_planting_date = "cc_plant") %>%
  rename(days_from_plant_to_bio_hrvst = "dayCount") %>%
  rename(
    field_acreage = "16. What is the acreage of this field?"
  ) %>%
  rename(
    years_with_cover_crops = "21. How many years have you been planting cover crops *in this field*?"
  ) %>%
  rename(
    cover_crop_planting_cost = "31. Estimated cover crop planting cost per acre in this field. Please use UW Extension Custom Rate Guide.(https://www.nass.usda.gov/Statistics_by_State/Wisconsin/Publications/WI-CRate20.pdf)"
  ) %>%
  rename(
    cover_crop_seed_cost = "30. Estimated cover crop seed cost for this field ($/acre)"
  ) %>%
  ...
  mutate(cash_crop_planting_date = ifelse(
    cash_crop_planting_date == 44819, "9/15", cash_crop_planting_date
  )) %>%
  mutate(
    cash_crop_planting_date = ifelse(
      cash_crop_planting_date == 44713,
      "6/1",
  ...
  mutate(
    cash_crop_planting_date = ifelse(
      cash_crop_planting_date == 45187, "9/18", cash_crop_planting_date
    )
  ) %>%
  mutate(
    cash_crop_planting_date = parse_date_time(
      paste(
        cash_crop_planting_date,
        "2022",
        sep = "/"
  ...
  # change n/a to NA
  mutate(
    tillage_equip_primary = ifelse(
      tillage_equip_primary == "n/a", NA, tillage_equip_primary
    )
  ) %>%
  mutate(
    tillage_equip_secondary = ifelse(
      tillage_equip_secondary == "n/a", NA, tillage_equip_secondary
    )
  ) %>%
  select(
    year,
...
counties <- read_sf(
  "~/Documents/exploring_soils/data/county_nrcs_a_wi.gdb/",
  layer = "county_nrcs_a_wi"
)

zips <- read_sf(
  "~/Documents/small_projects/wisc_cc/tl_2020_us_zcta520/tl_2020_us_zcta520.shp"
)

# We get lat/long from zip centroids
# Starts as NAD83 Conus Albers
...
dat_20_21 <- read_excel(
  paste0(dir_data, "MI Copy combined CC_citsci_2020-2021-grs.xlsx"),
  sheet = 1
)
...
dat_all <- dat_all %>%
  mutate(
    ...
    days_between_crop_hvst_and_cc_estd = as.numeric(
      ifelse(
        days_between_crop_hvst_and_cc_estd == ".",
        NA,
        days_between_crop_hvst_and_cc_estd
    ...
    years_with_cover_crops = as.numeric(
      ifelse(years_with_cover_crops == ".", NA, years_with_cover_crops)
    ),
    cover_crop_planting_cost = as.numeric(
      ifelse(
        cover_crop_planting_cost == ".", NA, cover_crop_planting_cost
      )
    ),
    cover_crop_seed_cost = as.numeric(
      ifelse(
        cover_crop_seed_cost == ".", NA, cover_crop_seed_cost
      )
    ),
    dominant_soil_texture = tolower(
      ifelse(dominant_soil_texture == ".", NA, dominant_soil_texture)
    )
  ) %>%
...
dat_all[dat_all$id == "53023-JW-22", "field_acreage"] <- 94
dat_all <- dat_all %>%
  mutate(
...
dat_precip <- read_tsv("./data/wisc_dat_precip_window.tsv")
dat_all <- dat_all %>%
  left_join(
    dat_precip,
    by = join_by("id" == "id")
