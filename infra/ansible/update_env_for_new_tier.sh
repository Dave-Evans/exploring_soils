...
if [ $1 == 'prod' ]; then
    ...
    sed -i "s/CSRF_TRUSTED_ORIGINS=.*/CSRF_TRUSTED_ORIGINS=evansgeospatial.com,www.evansgeospatial.com/g" .env
else
    sed -i "s/ALLOWED_HOSTS=.*/ALLOWED_HOSTS=$ipaddress,$1.evansgeospatial.com/g" .env
    sed -i "s/CSRF_TRUSTED_ORIGINS=.*/CSRF_TRUSTED_ORIGINS=$1.evansgeospatial.com/g" .env
fi